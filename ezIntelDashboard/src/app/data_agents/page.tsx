...
export default () => {
  ...
  const get_data_agents = async () => {
    const col = collection(db, "data_agents");
  ...
  return (
    <DefaultLayout>
      <Breadcrumb rootName=" " rootPath="/data_agents" pageName="Data Agents" />

      <Link
        href="/data_agents/new"
      ...
      <div className="flex flex-col gap-10">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
          {agents.map((agent) => (
            <div
              onClick={() => {
                router.push(`/data_agents/${agent.id}`);
              }}
              key={agent.id}
            ...
            >
              ...
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-300">
                {agent.data.logs?.length || 0} logs
              </p>

              {agent.data.config.keywords.length > 0 && (
                <ul className="mt-4">
                  <p>
                    Keywords:{" "}
                    <span className="text-sm">
                      {parseKeywords(agent.data.config.keywords)}
                    </span>
                  </p>
                </ul>
              )}
